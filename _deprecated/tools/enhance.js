...
const vocabIris = [
    ...
    "http://rdf.geospecies.org/methods/observationMethod.rdf",
    "http://rdfs.co/juso/",
    "http://rdfs.co/juso/0.1",
    ...
    "http://rdfs.co/bevon/",
    "http://rdfs.co/bevon/0.8",
    "http://rdfs.co/juso/kr/",
    "http://rdfs.co/juso/kr/0.1",
...
const operate = async (iris) => {
  const failedIris = [];
  for (let i = 0; i < iris.length; i++) {
    const iri = iris[i];
    const query = createQuery(iri);
    console.log(query);
    try {
      const resp = await axios.post(endpoint, `update=${query}`, axiosConfig);
      console.log(`DONE: ${iri}`);
    } catch (e) {
      console.log(`FAILED: ${iri}`);
      failedIris.push(iri);
    }
  }
